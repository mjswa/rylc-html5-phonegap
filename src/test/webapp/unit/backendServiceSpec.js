describe('backendService', function () {
  ...
  var backendBaseUrl = someServerUrl+backendBaseUri;
  ...
  function setAuthentication(username, password) {
    backendService.setCredentials(someServerUrl, username, password);
  ...
  describe("availableCars", function () {
    ...
    it("should call with GET method at URL", function () {
      ...
      expect($http).toHaveBeenCalledWith({
        url:backendBaseUrl + '/availableCars?cityId=' + someCity + '&startDate=' + startDate.toISOString() + '&endDate=' + endDate.toISOString() + '&maxPrice=' + maxPrice,
        method:'GET',
        headers:{
          'Authorization':authenticationHeader
        }
  ...
  describe('login', function () {
    it('should login and get the customer by username', function () {
      ...
      expectPromiseToUnpackResult(backendService.login(someServerUrl, username, password));
      authenticationHeader = "Basic " + Base64.encode(username + ':' + password);
      expect($http).toHaveBeenCalledWith({
        method:'GET',
        url:backendBaseUrl + '/customers?username=' + username,
